...
function file_get_html($url, $use_include_path = false, $proxy=null, $offset = -1, $maxLen=-1, $lowercase = true, $forceTagsClosed=true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT, $defaultSpanText=DEFAULT_SPAN_TEXT)
{    
//    if($proxy !== null){
//        exit($proxy);   
//    }
...
//    exit($url);
    ...
    {
        $contents = retrievePageContentAlt('http://'.$proxy, $url);
    }
...
/**
 * Return content using curl
 * 
 * @param type $proxy
 * @return type
 */
function retrievePageContentAlt($proxy, $url)
{
    $proxy = '27.50.19.101:3128';
...
//            $handle = fopen('yyyyyy.html', 'w');
//            fwrite($handle, $content);
//            fclose($handle);
//        
        echo($content);
        
    if($var = strpos($content, "To protect our users, we can't process your request right now." === false))
           exit('asdnjisayduiahdusidhsiusadsadsadasah');
